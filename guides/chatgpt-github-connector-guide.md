...
11. 创建 PR 后，默认不自动 merge；只有用户明确要求，或自动治理/同步任务的 prompt 明确要求且 PR 满足全部安全阈值时，才可使用 `expected_head_sha` 自动合入。
...
- 2026-07-04：自动治理/同步任务如明确要求闭环合入，只能在 changed files、路径、行数、mergeable 状态和 `expected_head_sha` 等安全阈值全部满足时自动 merge；否则 PR 必须保持 open 并列入 Manual attention。
